...
function Datepicker() {
	...
	this._inlineClass = "cal-wrap"; // The name of the inline marker class
...
$.extend( Datepicker.prototype, {
	...
	_generateHTML: function( inst ) {
		var maxDraw, prevText, prev, nextText, next, currentText, gotoDate,
			...
			printDate, dRow, tbody, daySettings, otherMonth, unselectable, tdDefault,
			tempDate = new Date(),
			// today = this._daylightSavingAdjust(
			// 	new Date( tempDate.getFullYear(), tempDate.getMonth(), tempDate.getDate() ) ), // clear time
			today = this._daylightSavingAdjust(
				new Date(this._get(inst, "tDay").getFullYear(), this._get(inst, "tDay").getMonth(), this._get(inst, "tDay").getDate())),
			isRTL = this._get( inst, "isRTL" ),
			...
			chkInTitle = this._get( inst, "chkInTitle"),
			chkOutTitle = this._get( inst, "chkOutTitle");
		...
		prev = ( this._canAdjustMonth( inst, -1, drawYear, drawMonth ) ?
			"<button class='btn-month prev-month ui-corner-all' type='button' data-handler='prev' data-event='click'" +
			" title='" + prevText + "'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "e" : "w" ) + "'>" + prevText + "</span></button>" :
			( hideIfNoPrevNext ? "" : "<button disabled class='btn-month prev-month ui-corner-all ui-state-disabled' type='button' title='" + prevText + "'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "e" : "w" ) + "'>" + prevText + "</span></button>" ) );
		...
		next = ( this._canAdjustMonth( inst, +1, drawYear, drawMonth ) ?
			"<button class='btn-month next-month ui-corner-all' type='button' data-handler='next' data-event='click'" +
			" title='" + nextText + "'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "w" : "e" ) + "'>" + nextText + "</span></button>" :
			( hideIfNoPrevNext ? "" : "<button class='btn-month next-month ui-corner-all ui-state-disabled' type='button' title='" + nextText + "'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "w" : "e" ) + "'>" + nextText + "</span></button>" ) );
		...
		for ( row = 0; row < numMonths[ 0 ]; row++ ) {
			...
			for ( col = 0; col < numMonths[ 1 ]; col++ ) {
				...
				if ( isMultiMonth ) {
					...
					if ( numMonths[ 1 ] > 1 ) {
						switch ( col ) {
							case 0: calender += " cal-inner cal-left";
								cornerClass = " ui-corner-" + ( isRTL ? "right" : "left" ); break;
							case numMonths[ 1 ] - 1: calender += " cal-inner calRight";
								cornerClass = " ui-corner-" + ( isRTL ? "left" : "right" ); break;
							default: calender += " cal-inner ui-datepicker-group-middle"; cornerClass = ""; break;
						}
					}
					calender += "'>";
				}
				...
				calender += "<div class='ui-datepicker-header ui-widget-header ui-helper-clearfix" + cornerClass + "'>" +
					...
					"</div><table class='calendar'><thead>" +
					"<tr>";
				thead = ( showWeek ? "<th class='ui-datepicker-week-col'>" + this._get( inst, "weekHeader" ) + "</th>" : "" );
				for ( dow = 0; dow < 7; dow++ ) { // days of the week
					day = ( dow + firstDay ) % 7;
					thead += "<th scope='col'" + ( ( dow + firstDay + 6 ) % 7 >= 6 ? " class='fRed'" : "" ) + ">" +
				...
				for ( dRow = 0; dRow < numRows; dRow++ ) { // create date picker rows
					...
					for ( dow = 0; dow < 7; dow++ ) { // create date picker days
						...
						tbody += "<td class='" +
							((daySettings[1]!="tdDefault"&&!unselectable && (dow + firstDay + 6) % 7 >= 6) ? " fRed" : "") + // highlight weekends
							...
							( unselectable ? "<span class='ui-state-default default'>" + printDate.getDate() + "</span>" : "<a class='ui-state-default" +
							( printDate.getTime() === today.getTime() ? " ui-state-highlight" : "" ) +
							( printDate.getTime() === currentDate.getTime() ? " ui-state-active" : "" ) + // highlight selected day
							( otherMonth ? " ui-priority-secondary" : "" ) + // distinguish dates from other months
							"'"
                            +(String(daySettings[1]).indexOf('is-start') != -1 ? " title='"+chkInTitle+"'":"")
							+(String(daySettings[1]).indexOf('is-end') != -1 ? " title='"+chkOutTitle+"'":"")
							+" href='#'>"
                            +( printDate.getTime() === today.getTime() ? "<em>Today</em>" : "" )
	...
	_generateMonthYearHeader: function( inst, drawMonth, drawYear, minDate, maxDate,
		...
		html += "</strong>"; // Close datepicker_header
		return html;
	},
...
function datepicker_bindHover( dpDiv ) {
	var selector = "button, .btn-month prev-month, .btn-month next-month, .calendar td a";
	return dpDiv.on( "mouseout", selector, function() {
			$( this ).removeClass( "ui-state-hover" );
			if ( this.className.indexOf( "btn-month prev-month" ) !== -1 ) {
				$( this ).removeClass( "ui-datepicker-prev-hover" );
			}
			if ( this.className.indexOf( "btn-month next-month" ) !== -1 ) {
